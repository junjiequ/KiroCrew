...
- **The proxy fails closed, and its cache follows the file.** `_get_app_secret`
  ...
  retry signal; `code` is what a client switches on). With no healthy snapshot
  the URL comes from the manifest, and a fixed port is the port a tracked
  child that failed its health probe is still bound to, so the same check runs
  by name (`backend_proxy_secret_matches`) before the fallback is used. A
  replacement in flight has no record at all — `stop_app_backend` pops it
  before the child is signalled and waited for (up to 5 s) — so the reconciler
  holds `backend_replacement(name)` from before that stop until the respawn
  attempt returns (and around a later tick's respawn), and the proxy refuses a
  name held there both at the gate and immediately before forwarding; and
...
- **The runtime reconciler replaces stale generations; it does nothing else to
  ...
  re-reads `app_enabled_state` immediately before spawning and again after,
  and the read is tri-state: a positive `True` proceeds, a positive `False`
  voids the replacement (or stops the new child if it already spawned), and
  `None` — unreadable metadata, a write in flight — DEFERS: the replacement
  is recorded as owed from the moment the old child is stopped, so an unknown
  read leaves that debt in place for the next tick rather than turning a
  stopped, enabled app into one that stays down. A CLI disable landing while
  the spawn is in flight therefore does not leave a disabled app serving until
  the next tick, and a transient read fault does not strand an enabled one. If
  a stop cannot release the record (an unsignallable adopted backend restores
  it), no competitor is spawned onto the same port.
...
- **A failing respawn is not retried every tick.** The boot path tries a
  ...
  or raised (a governance evaluation error propagates by design), so a
  crashing, port-blocked, execution-refused or policy-unevaluable replacement
  does not become a 15-second retry-and-log loop. The mark is what authorises the retry at all
  ...
  failed: it records the install identity (the `.app_secret` inode + mtime that
  `install_app`, the CLI's only path to new code, mints afresh; the dashboard's
  in-place `update_app` preserves the secret and restarts the backend itself),
  so a reinstall between two ticks is not held by
...
disable-during-respawn, disable-before-respawn, unknown-enablement-defers,
shutdown-in-gap-keeps-debt, unstoppable-adopted,
adopted-never-replaced, backoff, raised-spawn-backoff, in-flight-mark-spans-stop-to-start, in-flight-mark-cleared-on-raise, later-tick-respawn-marked, reinstall-between-ticks,
...
captured port, the unhealthy-stale-record 503 on the fixed-port fallback and
its unknown-by-name counterpart that forwards, the in-flight-replacement 503 at the gate and pre-forward, the rotation-following cache)
and `test_apps_backend_coverage.py` (digest binding
