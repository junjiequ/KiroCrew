...
async def handle_app_api_proxy(request: web.Request) -> web.StreamResponse:
    ...
    try:
        ...
        if target is not None:
            if target.secret_matches(secret) is False:
                return _app_restarting_response(name)
        elif backend_replacement_in_flight(name) or (
            backend_proxy_secret_matches(name, secret) is False
        ):
            # No healthy snapshot, so the URL above came from the manifest --
            # and a fixed port is the port the tracked child that failed its
            # health probe is still bound to. Its generation is stale all the
            # same; it is not reached through the by-name fallback either. Nor
            # is a child the reconciler is replacing right now: its record is
            # popped before it has exited, which reads as unknown by name.
            return _app_restarting_response(name)
    ...
    # now, or a replacement has begun since, the signed body is not sent to
    # whatever holds that port today.
    if backend_replacement_in_flight(name) or (
        target is not None and tracked_backend_target(name) != target
    ):
        return _app_restarting_response(name)
