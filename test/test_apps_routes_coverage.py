...
from kiro_crew.apps.backend import BackendTarget, backend_replacement
...
class TestApiProxyAuthorization:
    ...
    async def test_rotated_secret_is_a_retryable_503_and_never_forwarded(
        ...
        app = _make_app()
        # Any attempt to forward hits this and surfaces as a non-503 status.
        _fail_proxy_backend_with(
            app, AssertionError("a stale-generation request must never be forwarded")
        )
        async with TestClient(TestServer(app)) as client:
            resp = await client.get(f"/apps/{APP}/api/ping")
    ...
    async def test_target_that_changed_before_forwarding_is_not_sent_the_body(
        ...
        app = _make_app()
        _fail_proxy_backend_with(
            app, AssertionError("a replaced target must never receive the body")
        )
        async with TestClient(TestServer(app)) as client:
    ...
    @pytest.mark.asyncio
    async def test_unhealthy_stale_generation_is_not_reached_through_the_fixed_port(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """No healthy snapshot (the tracked child failed its health probe), so
        the URL comes from the manifest -- and a fixed port is the very port
        that child is still bound to. Spawned under the previous secret, it is
        refused like a healthy stale record, not forwarded to via the fallback."""
        home = _setup_env(tmp_path, monkeypatch)
        _install(tmp_path)
        enable_app(APP)
        (home / "apps" / APP / ".app_secret").write_text("rotated-key", encoding="utf-8")
        invalidate_app_secret_cache(APP)
        monkeypatch.setattr(routes_mod, "tracked_backend_target", lambda n: None)
        monkeypatch.setattr(
            routes_mod, "_resolve_app_backend_url", lambda n: "http://127.0.0.1:7801"
        )
        # The by-name check sees the unhealthy record spawned under "old-key".
        monkeypatch.setattr(
            routes_mod,
            "backend_proxy_secret_matches",
            lambda n, secret: secret == "old-key",
        )
        app = _make_app()
        _fail_proxy_backend_with(
            app, AssertionError("a stale generation must never be forwarded to")
        )
        async with TestClient(TestServer(app)) as client:
            resp = await client.get(f"/apps/{APP}/api/ping")
            assert resp.status == 503
    ...
    @pytest.mark.asyncio
    async def test_unknown_by_name_generation_still_forwards_via_fallback(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """No snapshot and no tracked spawned record (self-managed backend):
        the by-name check has no claim to make, and the fallback URL is used."""
        import aiohttp
        ...
        monkeypatch.setattr(routes_mod, "backend_proxy_secret_matches", lambda n, s: None)
        app = _make_app()
        _fail_proxy_backend_with(app, aiohttp.ClientError("refused"))
        async with TestClient(TestServer(app)) as client:
            resp = await client.get(f"/apps/{APP}/api/ping")
            assert resp.status == 502  # forwarded to the (dead) fallback backend

    @pytest.mark.asyncio
    async def test_a_replacement_in_flight_is_refused_not_resolved_by_name(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Mid-replacement the tracking record is already popped while the
        retiring child may still be bound to the manifest's fixed port. Both
        generation checks read unknown; the in-flight mark is what refuses."""
        home = _setup_env(tmp_path, monkeypatch)
        _install(tmp_path)
        enable_app(APP)
        (home / "apps" / APP / ".app_secret").write_text("rotated-key", encoding="utf-8")
        ...
        app = _make_app()
        _fail_proxy_backend_with(
            app, AssertionError("a replacement in flight must never be forwarded to")
        )
        async with TestClient(TestServer(app)) as client:
            with backend_replacement(APP):
                resp = await client.post(f"/apps/{APP}/api/run", json={"x": 1})
                assert resp.status == 503
                assert (await resp.json())["code"] == "app_restarting"

    @pytest.mark.asyncio
    async def test_a_replacement_beginning_across_the_await_points_is_refused(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """The snapshot was healthy and matched when captured; the reconciler
        began replacing the child while the body was being read. The final
        pre-forward decision sees the mark and refuses."""
        home = _setup_env(tmp_path, monkeypatch)
        _install(tmp_path)
        enable_app(APP)
        (home / "apps" / APP / ".app_secret").write_text("k", encoding="utf-8")
        ...
        # The snapshot above was captured and matched; with a healthy snapshot
        # the gate never consults the mark, so the mark being set models a
        # replacement that began after capture and is seen pre-forward only.
        monkeypatch.setattr(routes_mod, "backend_replacement_in_flight", lambda n: True)
        app = _make_app()
        _fail_proxy_backend_with(
            app, AssertionError("a replacement in flight must never be forwarded to")
        )
